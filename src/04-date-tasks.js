...
function timeSpanToString(startDate, endDate) {
  ...
  let milliseconds = parseInt(timeDiff % 1000);
  let seconds = parseInt((timeDiff / 1000) % 60);
  let minutes = parseInt((timeDiff / (1000 * 60)) % 60);
  let hours = parseInt((timeDiff / (1000 * 60 * 60)) % 24);
  while (String(milliseconds).length < 3) { milliseconds = `0${milliseconds}`; }
  while (String(seconds).length < 2) { seconds = `0${seconds}`; }
  while (String(minutes).length < 2) { minutes = `0${minutes}`; }
  while (String(hours).length < 2) { hours = `0${hours}`; }
