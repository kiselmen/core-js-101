...
function getRectangleString(width, height) {
  ...
  const LB = '└';
  const RB = '┘\n';
  return LT + T.repeat(width - 2) + RT
  + (L + I.repeat(width - 2) + R).repeat(height - 2)
  + LB + T.repeat(width - 2) + RB;
}
...
function encodeToRot13(str) {
  ...
  str.split('').forEach((item) => {
    const posittion = IN.indexOf(item);
    result += posittion !== -1 ? OUT[posittion] : item;
  });
  return result;
}
...
function isString(value) {
  ...
  if (value instanceof String) {
    ...
    result = (typeof value === typeof 'string') ? Boolean(1) : Boolean(0);
  }
  return result;
}
